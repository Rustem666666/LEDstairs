...
#define STRIP_LED_AMOUNT 50  // количество чипов WS2811/WS2812 на всех ступеньках. Для WS2811 кол-во чипов = кол-во светодиодов / 3
#define STEP_AMOUNT 10        // количество ступенек

// описание всех ступенек с возможностью подсветки ЛЮБЫХ ступенек в ночном режиме
Step steps[STEP_AMOUNT] = { 
{ 5, 0b0100100100100100 },   // первая ступенька 16 чипов, 0b0100100100100100 - каждый третий чип активен в ночном режиме
{ 5, 0b0000000000000000 },   // вторая ступенька 16 чипов, 0b0000000000000000 - не активен в ночном режиме
{ 5, 0b0000000000000000 },   // 3
{ 5, 0b0000000000000000 },   // 4
{ 5, 0b0000000000000000 },   // 5
{ 5, 0b0000000000000000 },   // 6
{ 5, 0b0000000000000000 },   // 7
{ 5, 0b0000000000000000 },   // 8
{ 5, 0b0000000000000000 },   // 9
{ 5, 0b0100100100100100 }
};
...
#define ORDER_GRB       // порядок цветов ORDER_GRB / ORDER_RGB / ориг ORDER_BRG
...
byte effectCounter;
byte colorCounterGlobal = 0; //Глобальная переменная colorCounter, нужная для запоминания цвета плавного включения
uint32_t timeoutCounter;
bool systemIdleState;
bool systemOffState;
int steps_start[STEP_AMOUNT];
bool stepFaderAction = false;
...
int InitPirPin = 0; //Иницирующий датчик движения, тот что подал сигнал на включение (0 - нет, 2 - конечный, 3 - начальный)
...
void handlePirSensor(PirSensor *sensor) {
  ...
  if (newState && !sensor->lastState) {
    ...
    //Принудительная остановка, если человек дошёл до второго датчика после анимации
    if (stepFaderAction == false && systemIdleState == false) {
      if (sensor->pin != InitPirPin && InitPirPin != 0) {
            Serial.println("STOP");
            timeoutCounter = -1;
            InitPirPin = 0;
            return;
    ...
    if (systemIdleState) {
      ...
      InitPirPin = sensor->pin;
    }
    Serial.print("Init startPIR sensor ");
    Serial.println(InitPirPin);
  }
